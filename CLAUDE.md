...
The harness is shipped as a Claude Code plugin at [`plugin/agentic-study-environment/`](plugin/agentic-study-environment/). It exposes four lifecycle skills that handle the entire sub-project workflow:

| Trigger phrasing | Skill | What it does |
| --- | --- | --- |
| "bootstrap a project for X", "create a sub-project to learn Y" | `agentic-study-environment:bootstrap` | Mint a new sub-project (`<name>/`, per-project CLAUDE.md and PROGRESS.md); register in root PROGRESS.md |
...
Each skill is self-sufficient and reads the files it needs (PROGRESS.md, sub-project CLAUDE.md, the relevant overlay) — there is no protocol duplicated in this file.
...
Install via the marketplace (works in any project, no clone required):
...
/                              ← host project root (this is where sub-projects accumulate)
  CLAUDE.md                    ← this file (host-project conventions; thin)
